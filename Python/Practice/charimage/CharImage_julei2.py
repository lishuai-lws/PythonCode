...
def img2strimg(frame, K=5):
    ...
    K：聚类数量，推荐的K为3或5。根据经验，3或5时可以较为优秀地处理很多图像了。若默认的K=5无法很好地表现原图，请修改为3进行尝试。
    若依然无法很好地表现原图，请换图尝试。 （ -_-|| ）    
    ---------------------------------
    聚类数目理论可以取大于等于3的任意整数。但水平有限，无法自动判断当生成的字符画可以更好地表现原图细节时，“黑暗”、“阴影”、”明亮“之间边界在哪。
    ...
    for rows in labels_picked:
        ...
        for cols in rows:
            if cols <= shadow_bound:
                ...
                cv2.putText(canvas, charSet[number%7],(x, y), cv2.FONT_ITALIC, 0.3, 1)
                number += 1
...
if __name__ == '__main__':
    ...
    imagePath = 'F:\\newcoder\\Python\\Practice\\image\\yhy02.jpg'
    # 字符图片保存地址
    imageCharPath = imagePath[:-4] + 'Char.jpg'
    img = cv2.imread(imagePath)
    # 若字符画结果不好，可以尝试更改K为3。若依然无法很好地表现原图，请换图尝试。 -_-||
    str_img = img2strimg(img)
    cv2.imwrite(imageCharPath, str_img)
    print('字符图片输出完成!')